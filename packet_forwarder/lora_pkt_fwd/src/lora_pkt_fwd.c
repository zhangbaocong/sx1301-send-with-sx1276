...
int main(void)
{
    ...
    char *global_cfg_path= "/home/pi/ytd/packet_forwarder/lora_pkt_fwd/global_conf.json"; /* contain global (typ. network-wide) configuration */
    char *local_cfg_path = "/home/pi/ytd/packet_forwarder/lora_pkt_fwd/local_conf.json"; /* contain node specific configuration, overwrite global parameters for parameters that are defined in both */
    char *debug_cfg_path = "/home/pi/ytd/packet_forwarder/lora_pkt_fwd/debug_conf.json"; /* if present, all other configuration files are ignored */
...
void thread_jit(void) {
    ...
    while (!exit_sig && !quit_sig) {
        ...
        //
        //printf("peek result:%d,pkt_index:%d\r\n",jit_result,pkt_index);
            ...
            if (pkt_index > -1) {
                ...
                if (jit_result == JIT_ERROR_OK) {
                    ...
                    if (pkt_type == JIT_PKT_TYPE_BEACON) {
                        ...
                        MSG_DEBUG(DEBUG_LOG, "beacon_pkt.freq_hz=%u (xtal_correct=%.15lf)\n", pkt.freq_hz, xtal_correct);
