...
void *spi1_target = NULL; /*! generic pointer to the SPI device */
...
/*
 * Binding DIO IRQ callback function
 */
void SX1276IoIrqInit( void )
{
	...
	if ( wiringPiISR (BUTTON_PIN, INT_EDGE_RISING, SX1276OnDio0Irq) < 0 ) {
		printf ("Unable to setup ISR IO\n");
		printf ("ISR IO NUM IS: %d\n", BUTTON_PIN);
	}
}
...
{
    uint8_t i;
	int spi1_stat = LGW_SPI_SUCCESS;

    /* open the SPI link */
    spi1_stat = lgw_spi_open_sx1278(&spi1_target);
    if (spi1_stat != LGW_SPI_SUCCESS) {
...
void SX1276SetTx( void )
{
	...
	printf("set RF_OPMODE_TRANSMITTER mode\r\n");
...
{
	lgw_spi_wb(spi1_target, LGW_SPI_MUX_MODE0, 0, addr, buffer, size);
}

void SX1276ReadBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
	lgw_spi_rb(spi1_target, LGW_SPI_MUX_MODE0, 0, addr, buffer, size);
}
...
void sx1276SendData( RadioModems_t modem, int8_t power, uint32_t fdev,
                        uint32_t freq, uint32_t bandwidth, uint32_t datarate,
                        uint8_t coderate, uint16_t preambleLen,
                        bool fixLen, bool crcOn, bool FreqHopOn,
                        uint8_t HopPeriod, bool iqInverted, uint8_t *data, uint8_t size )
{
		static uint32_t cnt=0;
		...
		printf("config:power,%d,bw:%d,sf:%d,cr:%d,preamblelen:%d,fixLen:%d,crcOn:%d,FreqHopOn:%d,HopPeriod:%d,iqInverted:%d\r\n",
						power,bandwidth,datarate,coderate,preambleLen,fixLen,crcOn ,FreqHopOn,HopPeriod,iqInverted);


		SX1276SetMaxPayloadLength(MODEM_LORA, maxPayload);

		SX1276Send( data, size );
		printf("send complete size:%d  cnt:%d\r\n", size, cnt++);
