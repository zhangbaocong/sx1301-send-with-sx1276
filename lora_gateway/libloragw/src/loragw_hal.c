...
int lgw_send(struct lgw_pkt_tx_s pkt_data)
{
    ...
    uint16_t tx_start_delay;
    uint32_t count_trig;
    uint32_t time_1301 = 0;
    if(pkt_data.modulation == MOD_LORA)     modem = MODEM_LORA;
    else if(pkt_data.modulation == MOD_FSK) modem = MODEM_FSK;
    else return LGW_HAL_ERROR;

    if(pkt_data.bandwidth == BW_125KHZ)     bw = 0;
    else if(pkt_data.bandwidth == BW_250KHZ) bw = 1;
    else if(pkt_data.bandwidth == BW_500KHZ) bw = 2;
    else return LGW_HAL_ERROR;

	if(pkt_data.datarate == DR_LORA_SF7) 		dataRate = 7;
    else if(pkt_data.datarate == DR_LORA_SF8) 	dataRate = 8;
	else if(pkt_data.datarate == DR_LORA_SF9) 	dataRate = 9;
	else if(pkt_data.datarate == DR_LORA_SF10) 	dataRate = 10;
	else if(pkt_data.datarate == DR_LORA_SF11) 	dataRate = 11;
	else if(pkt_data.datarate == DR_LORA_SF12) 	dataRate = 12;
	else return LGW_HAL_ERROR;

	if(pkt_data.coderate == CR_LORA_4_5)		codeRate = 1;
	else if(pkt_data.coderate == CR_LORA_4_6)	codeRate = 2;
	else if(pkt_data.coderate == CR_LORA_4_7)	codeRate = 3;
	else if(pkt_data.coderate == CR_LORA_4_8)	codeRate = 4;
	else return LGW_HAL_ERROR;
    ...
    memcpy(buffer,pkt_data.payload,pkt_data.size);

	lgw_get_trigcnt(&time_1301);
	printf("now time :%d\r\n",time_1301);
	bool tx_notch_enable = true;
    ...
    printf("txmode:%d,delayus:%d,now time :%d dif=%d\r\n",pkt_data.tx_mode,count_trig,time_1301,(count_trig-time_1301));

	sx1276SendData( modem, 20, pkt_data.f_dev,
                        pkt_data.freq_hz, bw, dataRate,
                        codeRate, pkt_data.preamble,
                        false, pkt_data.no_crc, false,
                        0, pkt_data.invert_pol, buffer, pkt_data.size );
...
int lgw_status(uint8_t select, uint8_t *code)
{
    ...
    if (select == TX_STATUS)
    {
        ...
        else if (SX1276.Settings.State == RF_IDLE)
        { /* bit 4 @1: TX programmed */
            *code = TX_FREE;
        }
        else if(SX1276.Settings.State == RF_TX_RUNNING)
